package leetcode_contests.weekly.weekly_212;
...
public class ArithmeticSubarrays {
    ...
    /*
    - Complexity Analysis:
    Time complexity: O(M * K (log K)) where M - is the length of l or r array and K - is the number of elements in the longest sequence
    Space complexity: O(log K) where K - is the number of elements in the longest sequence
     */
    public static List<Boolean> checkArithmeticSubarrays(int[] nums, int[] l, int[] r) {
        List<Boolean> result = new ArrayList<>();
        for (int i = 0; i < l.length; i++) {
            result.add(isArithmeticSeq(Arrays.copyOfRange(nums, l[i], r[i] + 1)));
        }

        return result;
    }

    private static boolean isArithmeticSeq(int[] a) {
        if (a.length < 3) {
            return true;
        }

        Arrays.sort(a);
        int arithmeticDiff = a[1] - a[0];
        for (int i = 2; i < a.length; i++) {
            if (a[i] - a[i - 1] != arithmeticDiff) {
                return false;
            }
        }
